...
import type { DocSegment, SegmentDiff } from '@/types';
...
// Scan every Word XML story (main body, headers/footers, notes, comments and glossary)
// instead of relying on HTML conversion, which silently omits several of these parts.
...
function replaceTextInElement(element: Element, before: string, after: string): boolean {
  ...
  nodes.forEach((node) => {
    ...
    node.textContent = `${left}${inserted ? '' : after}${right}`;
...
function nodeForSegment(source: XmlDocument, segment: DocSegment, diff: SegmentDiff): Element | null {
  if (segment.type === 'table') {
    const tables = Array.from(source.getElementsByTagNameNS(WORD_NS, 'tbl'));
    const table = tables[segment.meta?.nodeIndex ?? -1];
    if (!table) return null;
    const row = Array.from(table.getElementsByTagNameNS(WORD_NS, 'tr'))[diff.target?.row ?? -1];
    return row ? Array.from(row.getElementsByTagNameNS(WORD_NS, 'tc'))[diff.target?.column ?? -1] || null : null;
  }
  return Array.from(source.getElementsByTagNameNS(WORD_NS, 'p'))[segment.meta?.nodeIndex ?? -1] || null;
}

/** Applies text to the original Word XML. Styles, page setup, header/footer and table geometry remain untouched. */
export async function applyEditsToDocx(model: EditableDocx, diffs: SegmentDiff[]): Promise<string[]> {
  ...
  for (const diff of diffs) {
    ...
    const target = nodeForSegment(source, segment, diff);
    const after = diff.action === 'delete_text' ? '' : diff.after;
    if (target && replaceTextInElement(target, diff.before, after)) applied.add(diff.segmentId);
  }
...
function updateSegments(segments: DocSegment[], diffs: SegmentDiff[], applied: Set<string>): void {
  segments.forEach((segment) => {
    ...
    relevant.forEach((diff) => {
      ...
      if (segment.meta?.cells && diff.target?.row !== undefined && diff.target.column !== undefined) {
        const cell = segment.meta.cells[diff.target.row]?.[diff.target.column];
        if (cell !== undefined) segment.meta.cells[diff.target.row][diff.target.column] = cell.replace(diff.before, diff.after);
      }
    });
  });
}
...
export function findImageSegment(model: EditableDocx, image: HTMLImageElement): DocSegment | undefined {
  const source = normalizeDataUrl(image.currentSrc || image.src);
  return model.segments.find((segment) => segment.type === 'image'
    && segment.meta?.src
    && normalizeDataUrl(segment.meta.src) === source);
}
...
export async function replaceImageInDocx(model: EditableDocx, segment: DocSegment, file: File): Promise<string> {
  ...
  // Retain the original media filename and relationship: Word keeps its existing position.
  ...
  // Fix aspect ratio by adjusting the XML extents based on the new image size
  try {
...
export async function exportEditableDocx(model: EditableDocx): Promise<Blob> {
