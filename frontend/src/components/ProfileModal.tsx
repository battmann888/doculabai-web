  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="profile-modal animate-scale-in" role="dialog" aria-modal="true" aria-labelledby="profile-title">
        ...
        <label className="profile-field">Tanggal lahir<input type="text" inputMode="numeric" value={birthDate} onChange={(event) => setBirthDate(event.target.value)} placeholder="YYYY-MM-DD" pattern="\d{4}-\d{2}-\d{2}" title="Format: YYYY-MM-DD" /></label>
